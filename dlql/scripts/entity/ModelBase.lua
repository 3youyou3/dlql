local ModelBase = class("ModelBase", function()
...
function ModelBase:ctor(properties)
    ...
    -- 初始化组件
    if self.initComponents ~= nil then
        for k, c in pairs(self.initComponents) do
            self:addComponent(c)
            self[k.."__"] = self:getComponent(c)
        end
    end
end
